...
public class BallsConfigFragment extends PreferenceFragment
        implements Preference.OnPreferenceChangeListener{
    @Override
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        setPreferencesFromResource(R.xml.balls_config, rootKey);

        SwitchPreference prefRandomColor = findPreference("balls_random_color");
    ...
    public boolean onPreferenceChange(@NonNull Preference preference, Object newValue) {
        if(Objects.equals(preference.getKey(), "balls_random_color")){
            if(newValue == null){
                return true;
            }
            if((Boolean)newValue){
                ColorPickerPreference colorPref = findPreference("balls_color");
                assert colorPref != null;
                //colorPref.setColor(0xff000000);
                colorPref.setVisible(false);

                PalettePreference palettePref = findPreference("balls_palette");
